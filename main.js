  ...
  try {
    ...
    if (hoursDiff > 1) {
      ...
      const linkStmt = db.prepare(`
        INSERT INTO links (id, source_note_id, source_line_index, target_note_id, target_line_index, type, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      console.log('Created new version:', newNote);
      return newNote;
    } else {
...
ipcMain.handle('db:getAllLinks', async () => {
  try {
    ...
    console.log('Retrieved links:', links.length);
    return links;
  } catch (error) {
